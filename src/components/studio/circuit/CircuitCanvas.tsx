  ...
  return (
    <div className={`w-full h-full flex ${themeMode === 'dark' ? 'bg-[#0A0E17]' : 'bg-[#F8FAFC]'} text-white overflow-hidden relative select-none`}>
      ...
      <div ref={reactFlowWrapper} className="flex-1 flex flex-col relative h-full">
        ...
        {edges.length > 0 && (
          <div className="h-32 bg-slate-900/95 border-t border-slate-800 flex flex-col p-2.5 font-mono text-xs overflow-hidden">
            <div className="flex items-center justify-between pb-1 text-[11px] text-slate-400 font-bold">
              <span className="flex items-center gap-1 text-sky-400">
                <Zap className="w-3.5 h-3.5" /> Circuit Netlist Wires ({edges.length})
              </span>
              <span>Drag wires from pin to pin • Click wire trash to delete</span>
            </div>
